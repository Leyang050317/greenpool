<x-ratings.shell>
    <div class="min-h-screen bg-[#F8FAFC] px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-4xl">
            ...
            <div class="mt-6"><h1 class="text-2xl font-bold text-slate-900">{{ ucfirst($targetRole) }} Profiles & Reviews</h1><p class="mt-1 text-sm text-slate-400">View profiles, average ratings, and reviews from your accepted bookings.</p></div>
            ...
            <div class="mt-5 space-y-3">
                @forelse($people as $person)
                    <article class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center"><div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-green-600 text-sm font-bold text-white">{{ strtoupper(substr($person->name,0,2)) }}</div><div class="min-w-0 flex-1"><h2 class="font-semibold text-slate-900">{{ $person->name }}</h2><p class="mt-1 text-sm text-slate-500"><span class="text-amber-400">★</span> {{ number_format((float) $person->ratings_received_avg_score, 1) }} · {{ $person->ratings_received_count }} {{ Str::plural('review', $person->ratings_received_count) }}</p></div><a href="{{ route('ratings.received', $person) }}" class="w-full rounded-xl border border-slate-200 px-4 py-2 text-center text-sm font-semibold text-slate-600 hover:bg-slate-50 sm:w-auto">View Profile & Reviews</a></article>
