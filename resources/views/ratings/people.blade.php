        ...
        <div class="mx-auto max-w-4xl">
            ...
            <div class="mt-5 space-y-3">
                @forelse($people as $person)
                    <article class="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-green-600 text-sm font-bold text-white">{{ strtoupper(substr($person->name,0,2)) }}</div><div class="min-w-0 flex-1"><h2 class="font-semibold text-slate-900">{{ $person->name }}</h2><p class="mt-1 text-sm text-slate-500"><span class="text-amber-400">★</span> {{ number_format((float) $person->ratings_received_avg_score, 1) }} · {{ $person->ratings_received_count }} reviews</p></div><a href="{{ route('ratings.received', $person) }}" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">View Reviews</a></article>
