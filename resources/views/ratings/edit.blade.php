...
@endphp
<x-ratings.shell>
    <div class="min-h-screen bg-[#F8FAFC] px-4 py-10" x-data='{ score: {{ old('score', $rating->score) }}, comment: @json(old('comment', $rating->comment ?? '')), selectedFeedback: [], suggestions: @json($suggestions), toggleFeedback(item) { this.selectedFeedback = this.selectedFeedback.includes(item) ? this.selectedFeedback.filter(value => value !== item) : [...this.selectedFeedback, item]; this.comment = this.selectedFeedback.join(". "); } }' x-init="selectedFeedback = (suggestions[score] || []).filter(item => comment.includes(item))">
        <div class="mx-auto max-w-xl">
            <a href="{{ route('ratings.history') }}" class="text-sm font-medium text-slate-500 hover:text-slate-700">← Back to Rating History</a>
            <div class="mt-7"><h1 class="text-2xl font-bold text-slate-900">Update Rating</h1><p class="mt-1 text-sm text-slate-400">Update the rating and feedback you submitted.</p></div>
            <form method="POST" action="{{ route('ratings.update', $rating) }}" class="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">@csrf @method('PATCH')
                <div class="flex items-center gap-4 rounded-xl bg-slate-50 p-4"><div class="flex h-11 w-11 items-center justify-center rounded-full bg-green-600 text-sm font-bold text-white">{{ strtoupper(substr($rating->reviewee->name, 0, 2)) }}</div><div class="min-w-0"><div class="truncate font-semibold text-slate-900">{{ $rating->reviewee->name }}</div><div class="text-xs text-slate-400">{{ ucfirst($rating->reviewee->role) }} · {{ $booking->trip->completed_at->format('j M Y') }}</div></div></div>
                <fieldset class="mt-7 text-center"><legend class="w-full text-sm font-semibold text-slate-700">Update your rating</legend><div class="mt-3 flex justify-center gap-1 sm:gap-2">@for($i=1;$i<=5;$i++)<button type="button" @click="if (score !== {{$i}}) { selectedFeedback = []; comment = '' }; score={{ $i }}" class="text-4xl leading-none transition hover:scale-110 sm:text-5xl" :class="score >= {{ $i }} ? 'text-amber-400' : 'text-slate-200'" aria-label="{{ $i }} stars">★</button>@endfor</div><input type="hidden" name="score" :value="score"><p class="mt-3 text-sm font-semibold text-green-600" x-text="['','Poor','Fair','Good','Very good','Excellent'][score]"></p>@error('score')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</fieldset>
                <div class="mt-6 border-t border-slate-100 pt-5"><p class="text-sm font-semibold text-slate-700">Quick feedback <span class="font-normal text-slate-400">(select one or more)</span></p><div class="mt-3 flex flex-wrap gap-2"><template x-for="suggestion in (suggestions[score] || [])" :key="suggestion"><button type="button" @click="toggleFeedback(suggestion)" class="rounded-full border px-3 py-2 text-xs font-medium transition" :class="selectedFeedback.includes(suggestion) ? 'border-green-500 bg-green-50 text-green-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'" :aria-pressed="selectedFeedback.includes(suggestion)" x-text="suggestion"></button></template></div><label for="comment" class="mt-5 block text-sm font-semibold text-slate-700">Comments &amp; Feedback <span class="font-normal text-slate-400">(optional)</span></label><textarea id="comment" name="comment" x-model="comment" maxlength="300" rows="4" class="mt-2 w-full rounded-xl border-slate-300 text-sm focus:border-green-500 focus:ring-green-500" placeholder="Select quick comments or write your own..."></textarea><div class="mt-1 text-right text-xs text-slate-400"><span x-text="comment.length"></span> / 300</div>@error('comment')<p class="text-sm text-red-600">{{ $message }}</p>@enderror</div>
                ...
                <button class="mt-5 w-full rounded-xl bg-[#22C55E] py-3 text-sm font-bold text-white disabled:opacity-40" :disabled="score===0">Update Rating</button>
            </form>
        </div>
    </div>
</x-ratings.shell>
