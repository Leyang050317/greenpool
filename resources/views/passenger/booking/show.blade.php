...
@section('content')
    @php
        $trip = $booking->trip;
        $displayStatus = $booking->booking_status === 'Accepted' && $trip->status !== 'Scheduled' ? $trip->status : $booking->booking_status;
        $statusStyle = match ($displayStatus) { 'Accepted' => 'bg-green-50 text-green-700', 'In Progress' => 'bg-orange-50 text-orange-700', 'Completed' => 'bg-slate-100 text-slate-700', 'Pending' => 'bg-amber-50 text-amber-700', default => 'bg-red-50 text-red-700' };
    ...
    <div class="min-h-screen bg-[#F8FAFC] px-4 py-8 sm:px-6 lg:px-8"><div class="mx-auto max-w-5xl">
        ...
        @elseif($booking->booking_status === 'Accepted' && $trip->status === 'Scheduled')
            <section class="mb-6 rounded-2xl border border-green-200 bg-green-50 p-5"><h2 class="font-semibold text-green-900">Upcoming trip</h2><p class="mt-1 text-sm text-green-800">Your booking is confirmed for {{ $trip->departure_at->format('d M Y, g:i A') }}.</p></section>
        @endif

        <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_19rem]"><div class="space-y-6">
            <section class="rounded-2xl border border-gray-100 bg-white p-5"><h2 class="mb-4 text-sm font-semibold text-gray-900">Route and trip information</h2><dl class="grid gap-4 sm:grid-cols-2">@foreach([['map-pin','Departure',$trip->departure_location],['flag','Destination',$trip->destination],['calendar','Departure',$trip->departure_at->format('d M Y, g:i A')],['map-pin','Your pickup point',$booking->pickup_point],['route','Estimated distance',$trip->estimated_distance_km ? $trip->estimated_distance_km.' km' : '—'],['clock','Estimated duration',$trip->estimated_duration_seconds ? ceil($trip->estimated_duration_seconds / 60).' min' : '—']] as [$icon,$label,$value])<div><dt class="flex items-center gap-1.5 text-xs font-medium text-gray-500"><x-icons.lucide :name="$icon" class="h-3.5 w-3.5" />{{ $label }}</dt><dd class="mt-1 text-sm font-semibold text-gray-900">{{ $value }}</dd></div>@endforeach</dl></section>
            ...
            @if($trip->status === 'In Progress' && $booking->booking_status === 'Accepted')
                <section class="rounded-2xl border border-gray-100 bg-white p-5"><h2 class="mb-4 text-sm font-semibold text-gray-900">Journey progress</h2><ol class="space-y-4">@forelse($acceptedPickups as $pickup)<li class="flex items-center gap-3"><span class="flex h-7 w-7 items-center justify-center rounded-full {{ $pickup->picked_up_at ? 'bg-green-600 text-white' : ($nextPickup?->is($pickup) ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-500') }} text-xs font-bold">{{ $pickup->picked_up_at ? '✓' : ($nextPickup?->is($pickup) ? '→' : '○') }}</span><div><p class="text-sm font-medium text-slate-900">Pickup {{ $loop->iteration }}{{ $pickup->is($booking) ? ' (your pickup)' : '' }}</p><p class="text-xs text-slate-500">{{ $pickup->picked_up_at ? 'Picked up '.$pickup->picked_up_at->format('g:i A') : ($nextPickup?->is($pickup) ? 'Current / waiting' : 'Upcoming') }}</p></div></li>@empty<li class="text-sm text-slate-500">Pickup progress is not available yet.</li>@endforelse<li class="flex items-center gap-3"><span class="flex h-7 w-7 items-center justify-center rounded-full {{ $nextPickup ? 'bg-slate-100 text-slate-500' : 'bg-orange-100 text-orange-700' }} text-xs font-bold">○</span><div><p class="text-sm font-medium text-slate-900">Destination</p><p class="text-xs text-slate-500">{{ $nextPickup ? 'Upcoming' : 'Next stop' }}</p></div></li></ol></section>
