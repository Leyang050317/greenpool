<x-app-layout>
    ...
    <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8" x-data="{
            ...
            <div class="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <button type="button" @click="showDeleteModal = true" class="inline-flex items-center px-4 py-2 border border-red-200 text-sm font-medium rounded-lg text-red-600 bg-white hover:bg-red-50 transition-colors">Deactivate Account</button>
                    </div>
                </div>
                </section>
            </div>
        ...
        <div x-cloak x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" x-transition.opacity><div @click.away="showDeleteModal = false" class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"><h3 class="text-lg font-bold text-gray-950">Deactivate your account?</h3><p class="mt-2 text-sm leading-6 text-gray-600">You will be signed out. Your account can be reactivated within 30 days; after that, personal details are permanently anonymized while trip and payment records stay protected.</p>@if ($user->usesGoogleAuthentication())<a href="{{ route('account.deactivate.google') }}" class="mt-6 inline-flex w-full items-center justify-center rounded-lg bg-[#2E7D32] px-4 py-2 text-sm font-semibold text-white hover:bg-[#256b29]">Continue with Google to confirm</a>@else<form method="POST" action="{{ route('profile.destroy') }}" class="mt-5">@csrf @method('DELETE')<label for="delete_password" class="sr-only">Current password</label><input id="delete_password" name="password" type="password" required autocomplete="current-password" placeholder="Current password" class="block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-red-500 focus:ring-red-500"><x-input-error :messages="$errors->userDeletion->get('password')" class="mt-2" /><div class="mt-6 flex justify-end gap-3"><button type="button" @click="showDeleteModal = false" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</button><button type="submit" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">Deactivate Account</button></div></form>@endif</div></div>
    </div>
</x-app-layout>
