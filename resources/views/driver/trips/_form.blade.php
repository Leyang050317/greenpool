...
<form method="POST" action="{{ $isEditing ? route('driver.trips.update', ['trip' => $trip, 'return_to' => $returnTo]) : route('driver.trips.store') }}" class="max-w-2xl space-y-6" x-data="tripLocationForm({{ Js::from(route('driver.trips.locations.autocomplete')) }}, {{ Js::from($value('departure_location', $trip->departure_location ?? '')) }}, {{ Js::from($value('destination', $trip->destination ?? '')) }}, {{ Js::from(old('departure_place_id', '')) }}, {{ Js::from(old('destination_place_id', '')) }})" x-ref="tripForm" @submit.prevent="locationsValid() && (confirmOpen = true)">
    @csrf
        ...
        <div class="space-y-5">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="relative">
                    ...
                    <input id="departure_location" x-model="departure.text" @input="search('departure')" @focus="departure.open = true" autocomplete="off" maxlength="255" required class="block w-full rounded-xl border-gray-200 text-sm focus:border-[#16A34A] focus:ring-[#16A34A]" placeholder="Search a Malaysian address or place" />
                ...
                <div class="relative">
                    ...
                    <input id="destination" x-model="destination.text" @input="search('destination')" @focus="destination.open = true" autocomplete="off" maxlength="255" required class="block w-full rounded-xl border-gray-200 text-sm focus:border-[#16A34A] focus:ring-[#16A34A]" placeholder="Search a Malaysian address or place" />
...
<script>
    window.tripLocationForm = (endpoint, departureText, destinationText, departurePlaceId, destinationPlaceId) => ({
        confirmOpen: false, discardOpen: false, timer: null,
        ...
        locationsValid() { return [this.departure, this.destination].every((location) => location.placeId || location.text === location.initialText); },
        async search(field) {
