    ...
    <div class="min-h-screen bg-[#F8FAFC] px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-4xl">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div><h1 class="text-2xl font-bold text-slate-900">Notifications</h1><p class="mt-1 text-sm text-slate-400">Rating and account updates.</p></div>
            ...
            <div class="mt-6 space-y-3">
                @forelse($notifications as $notification)
                    <a href="{{ route('notifications.open', $notification->id) }}" class="flex gap-4 rounded-2xl border p-5 shadow-sm transition hover:border-green-300 {{ $notification->read_at ? 'border-slate-200 bg-white' : 'border-green-200 bg-green-50/60' }}">
                        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-amber-100 text-amber-500"><x-icons.lucide name="star" class="h-5 w-5 fill-current" /></span>
                        <span class="min-w-0 flex-1"><span class="block font-semibold text-slate-900">New rating received</span><span class="mt-1 block text-sm text-slate-600">{{ $notification->data['message'] }}</span><span class="mt-2 block text-xs text-slate-400">{{ $notification->created_at->diffForHumans() }}</span></span>
