<x-support.shell page-title="Settings">
    <div class="min-h-screen bg-[#F8FAFC] px-4 py-8 sm:px-6 lg:px-8"><div class="mx-auto max-w-4xl">
        ...
        <form method="POST" action="{{ route('settings.notifications.update') }}" class="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">@csrf @method('PUT')
            <div class="flex items-start gap-3"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-green-50 text-[#2E7D32]"><x-icons.lucide name="bell" class="h-5 w-5" /></span><div><h2 class="text-lg font-bold text-slate-900">Notification preferences</h2><p class="mt-1 text-sm text-slate-500">Choose the updates you want to receive inside GreenPool.</p></div></div>
            <div class="mt-5 divide-y divide-slate-100">@foreach ([
                'trip_updates' => ['Trip updates', 'Changes, reminders, and status updates for your trips.'],
                'booking_updates' => ['Booking updates', 'Booking requests, acceptance, rejection, and cancellations.'],
                'payment_updates' => ['Payment updates', 'Payment due, completed, and received updates.'],
                'message_alerts' => ['Message alerts', 'Let you know when another GreenPool user sends a message.'],
                'rating_reminders' => ['Rating reminders', 'Reminders to leave a rating after a completed trip.'],
                'attraction_updates' => ['Tourist attraction updates', 'Saved attraction and destination-related updates.'],
