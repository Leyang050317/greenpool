@php
    ...
    $bottomNavigation = [
        ['label' => 'Settings', 'icon' => 'settings', 'href' => '#', 'active' => request()->routeIs('passenger.settings.*')],
        ['label' => 'Help', 'icon' => 'circle-help', 'href' => '#', 'active' => request()->routeIs('passenger.help.*')],
    ];
...
<aside {{ $attributes->merge(['class' => 'flex h-full flex-col border-r border-slate-200 bg-white text-slate-800']) }} aria-label="Passenger navigation">
    <div class="flex h-[84px] shrink-0 items-center border-b border-slate-100 px-4">
        <a href="{{ route('passenger.home') }}" class="flex min-w-0 items-center rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2E7D32] focus-visible:ring-offset-2" aria-label="GreenPool dashboard">
            ...
            <span x-show="sidebarExpanded || mobileDrawerOpen" x-transition.opacity.duration.150ms class="ml-4 whitespace-nowrap text-xl font-semibold text-[#1F2D3D]">GreenPool</span>
        </a>

        <button type="button" class="ml-auto flex h-10 w-10 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2E7D32] focus-visible:ring-offset-2 md:hidden" @click="closeMobileDrawer()" aria-label="Close navigation menu">
            ...
            @foreach ($mainNavigation as $item)
                <li>
                    <a href="{{ $item['href'] }}" @click="if (mobileDrawerOpen) closeMobileDrawer()" @if ($item['active']) aria-current="page" @endif title="{{ $item['label'] }}" class="group relative flex h-13 min-h-[52px] items-center rounded-xl px-3 text-[15px] font-medium transition duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2E7D32] focus-visible:ring-offset-2 {{ $item['active'] ? 'bg-green-50 text-[#2E7D32]' : 'text-slate-700 hover:bg-slate-100 hover:text-slate-950' }}">
                        ...
                        <span x-show="!sidebarExpanded && !mobileDrawerOpen" class="pointer-events-none absolute left-full z-50 ml-3 hidden whitespace-nowrap rounded-md bg-slate-900 px-2.5 py-1.5 text-xs font-medium text-white shadow-sm group-hover:block group-focus-visible:block" role="tooltip">{{ $item['label'] }}</span>
                    </a>
                </li>
            @endforeach
        </ul>
        ...
        <nav aria-label="Support">
            <ul class="space-y-1">
                @foreach ($bottomNavigation as $item)
                    <li>
                        <a href="{{ $item['href'] }}" @click="if (mobileDrawerOpen) closeMobileDrawer()" title="{{ $item['label'] }}" class="group relative flex min-h-[48px] items-center rounded-xl px-3 text-[15px] font-medium text-slate-700 transition duration-150 hover:bg-slate-100 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2E7D32] focus-visible:ring-offset-2">
                            <span class="flex h-7 w-7 shrink-0 items-center justify-center text-slate-400 group-hover:text-slate-600"><x-icons.lucide :name="$item['icon']" /></span>
                            <span x-show="sidebarExpanded || mobileDrawerOpen" x-transition.opacity.duration.150ms class="ml-3 whitespace-nowrap">{{ $item['label'] }}</span>
                            <span x-show="!sidebarExpanded && !mobileDrawerOpen" class="pointer-events-none absolute left-full z-50 ml-3 hidden whitespace-nowrap rounded-md bg-slate-900 px-2.5 py-1.5 text-xs font-medium text-white shadow-sm group-hover:block group-focus-visible:block" role="tooltip">{{ $item['label'] }}</span>
                        </a>
                    </li>
